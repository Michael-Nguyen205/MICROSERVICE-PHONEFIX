...
public class AuthenticationService {
    ...
    public AuthenticationResponse authenticate(AuthenticationRequest request) {
        ...
        log.error(authenticationToken.toString());
    ...
    private SignedJWT verifyToken(String token, boolean isRefresh) throws JOSEException, ParseException {
        ...
        // 1 JWSVerifier là một công cụ để xác minh chữ ký của JWT.
        // Ở đây, MACVerifier được sử dụng với khóa bí mật SIGNER_KEY.
        // tạo đối tượng JWSVerifier để tí nữa check chữ jys
        JWSVerifier verifier = new MACVerifier(SIGNER_KEY.getBytes());

        // 2 chuyển đổi chuỗi token thành một đối tượng SignedJWT để bạn có
        // thể truy cập các phần của token như claims và chữ ký.
        SignedJWT signedJWT = SignedJWT.parse(token);

        // 3 xác minh chữ ký của token để đảm bảo rằng nó
        // không bị thay đổi kể từ khi được ký.
